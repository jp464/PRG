...
class visuals():
    ...
    def segmented_bar(self, df, labels, questions, category_names, ht):
        ...
        category_colors = plt.colormaps['bone'](
    ...
    def single_bar(self, values, labels, xlabel, ylabel, title, color=["#1e76b4"]): # make arguments optional?
       fig, ax = plt.subplots()
    ...
    def double_bar(self, values1, values2, labels, lab1, lab2, xlabel, ylabel, title, ht, rotate=0):
        x = np.arange(len(labels))  # the label locations
        width = 0.35  # the width of the bars
        fig, ax = plt.subplots(figsize=(20, ht))
        bar1 = ax.bar(x - width/2, values1, width, label=lab1)
        bar2 = ax.bar(x + width/2, values2, width, label=lab2)
        ax.set_xlabel(xlabel)
        ax.set_ylabel(ylabel)
        ax.set_title(title)
